...
1. **Find Network Name**: After deployment, in your Coolify project:
   ...
   > **Alternative: Find network ID via Docker CLI**
   > SSH into your server and run:
   > ```bash
   > docker network ls
   > ```
   > Look for a network with a long alphanumeric name matching your Coolify project.
   >
   > Or inspect a running container directly:
   > ```bash
   > # List running containers to find a container name
   > docker ps --format "{{.Names}}"
   >
   > # Get the network name from a specific container
